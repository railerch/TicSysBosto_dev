...
<div style="background: #505050;padding: 0.5em;border-radius: 1em;box-shadow: 0px 0px 10px rgb(0,0,0);border-width: 1px;border-style: none;border-top-style: none;border-right-style: none;border-bottom-style: none;color: #d7d7d7;">
    ...
    <div id="formulario">
        <form id="taskForm" style="background: #7c7c7c;border-radius: 10px;padding: 1em;">
            <h3>Datos de la tarea<br></h3>
            <fieldset>
                <div class="form-group form-inline">
                    <select class="form-control" name="analista" id="analista" style="min-width:30%">
    ...
    <div class="text-light table-striped" style="background: #ffffff;margin-bottom: 1em;width: 100%;margin-top: 1em;padding:0.5em">
        <table id="tasksList" class="table">
            <thead>
                <tr style="text-align: center;background: #5b5b5b;color: rgb(255,255,255);">
            ...
            <tbody>
                <?php while ($tarea = $stmt_tsk->fetch()) { ?>
                    <tr id="tsk<?php echo $tarea['id_tarea'] ?>" style="text-align: center;">
                        ...
                        <?php } ?>
                        <td data-cell="analista"><?php echo $tarea['analista'] ?></td>
                        ...
                        <td>
                            <div class="btn-toolbar d-flex flex-row justify-content-center">
                                <div class="btn-group" role="group">
                                    <a class="btn btn-outline-primary btn-sm verTarea" data-task-id="tsk<?php echo $tarea['id_tarea'] ?>" role="button" data-toggle="modal" data-bs-tooltip="" title="Ver tarea" href="#/" data-target="#verTarea">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                    <?php if ($tarea['estatus'] != 'Verificada') { ?>

                                        <?php if ($tarea['estatus'] == 'Finalizada') { ?>
                                            <a class="btn btn-outline-success btn-sm estatusBtn" data-task-id="tsk<?php echo $tarea['id_tarea'] ?>" data-task-status="Verificada" role="button" title="Verificar tarea" href="#/">
                                                <i class="fa fa-check"></i>
                                            </a>
                                        <?php } else { ?>
                                            <?php if ($tarea['estatus'] == 'Pendiente' || $tarea['estatus'] == 'En espera') { ?>
                                                <a class="btn btn-outline-warning btn-sm editarTarea" data-task-id="tsk<?php echo $tarea['id_tarea'] ?>" role="button" data-toggle="modal" data-bs-tooltip="" title="Editar tarea" href="#/" data-target="#editarTarea">
                                                    <i class="fa fa-edit"></i>
                                                </a>

                                                <a class="btn btn-outline-danger btn-sm eliminarTarea" data-task-id="<?php echo $tarea['id_tarea'] ?>" role="button" data-toggle="modal" data-bs-tooltip="" title="Eliminar tarea" href="#/" data-target="#eliminarTarea">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="color: #333;">
                ...
                <ul class="list-unstyled">
                    <li><strong>Analista: </strong><span data-tag="analista"></span></li>
                    <li>
            ...
            <div class="modal-footer"><button class="btn btn-primary" type="button" data-dismiss="modal" data-cancel="ok">CERRAR</button>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form id="upTaskForm">
                ...
                <div class="modal-body" style="color: #333;">
                    <strong>Analista: </strong><span data-tag="analista"></span>
                    <textarea class="form-control" name="descripcion" data-tag="descripcion" cols="30" rows="10" placeholder="Descripción de la tarea" style="min-width:100%; max-width:100%; min-height:10em; max-height:10em;">
                        </textarea>
                </div>

                <div class="modal-footer"><button id="actualizarBtn" class="btn btn-primary" type="button" data-dismiss="modal" data-cancel="ok">ACTUALIZAR</button>
                </div>
...
<div class="modal fade" role="dialog" tabindex="-1" id="eliminarTarea" style="color: #212529;">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title"><i class="fa fa-trash-o"></i>&nbsp;Eliminar tarea<br></h4><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
            </div>
            ...
            <div class="modal-footer">
                ...
                <button class="btn btn-danger eliminarConfirmacion" data-task-id="" type="button" data-dismiss="modal">SI</button></a>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(".verTarea").click(function() {
            ...
            $("#verTarea span[data-tag=analista]").text($(`#${id_tarea} td[data-cell=analista]`).text());
        ...
        $(".editarTarea").click(function() {
            ...
            $("#editarTarea select[data-tag=valoracion]").val($(`#${id_tarea} td[data-cell=valoracion]`)
                .text());
            $("#editarTarea span[data-tag=analista]").text($(`#${id_tarea} td[data-cell=analista]`).text());
